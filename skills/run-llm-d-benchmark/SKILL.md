...
description: Runs a workload against an already deployed llm-d stack (high-performance distributed LLM inference), using one of several available benchmark harnesses.
---
...
If a NAMESPACE environment variable is specified, the llm-d stack is assumed to be deployed in that NAMESPACE. Otherwise, use the `oc project` command to see if an oc project is defined. If so, the stack is assumed to be deployed in the current oc project. If an oc project does not exist, ask the user to provide the NAMESPACE where it is deployed. 

Verify that the stack is indeed deployed in the detected or provided namespace using kubectl commands. If you cannot locate the stack, refer the user to the llm-d-kubernetes deployment skill to deply a stack.

If you detected the deployment in an oc project and the NAMESPACE env var is not defined, set it using the export command.

### Step 2: Verify the existence of a storage class

Make sure Kubernetes storage class for PVCs exists in the namespace, using the command `kubectl get pvc -n $NAMESPACE`. If it doesn't exist, create a PVC after asking the user to provide its required name. If it exists, set the BENCHMARK_PVC environment variable to the name of the PVC.
...
### Step 5: Determine which workload to use

Ask the user to select one of the following options:

1. Confirm the existing workload specified in the config file
2. Provide a link or a path to to another workload
3. provide a workload description. In this case, you should automatically generate a workload based on it.

If a non-default workload is used, update the instantiated configuration file with the selected or generated workload.

### Step 6: Obtain the benchmarking script 
...
### Step 7: Verify the model to use

Ask the user to eithre confirm the model appearing in the selected configuration file, or select another model. If the user selects to change the model name, list the available models in the cluster using the command `kubectl get deployment ms-inference-scheduling-llm-d-modelservice-decode -n rachelt-benchmark -o yaml`

If a non-default model is selected, update the instantiated configuration file with the selected model.

### Step 8: Verify the final configuration

Display the instantiated configuration file to the user for verification. If user wants to change any of the configuration parameters, update the instantiated configuration file based on user feedback.
...
Ask the user whether any analysis of raw results is requested. For example, tcreate specific graphs or tables from raw metric reporting. if the user provides an analysis description, create corresponding analysis scripts, run them, and store them along with their results in the `analysis` directory inside the `results` directory.
