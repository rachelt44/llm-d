...
Not removed unless you choose below:
  ...
  PVCs: <list with sizes or "none found">
  Namespace: <NAMESPACE>
...
## Step 6: Optionally Remove PVCs

Ask the user:

> "The following PVCs exist in `<NAMESPACE>`. These may contain benchmark results or KV-cache data. Do you want to delete any of them?"
> `<list of PVCs with sizes>`

Only delete PVCs the user explicitly approves. If approving all:
```bash
kubectl delete pvc --all -n $NAMESPACE
```

If approving individual PVCs:
```bash
...
## Step 7: Optionally Delete the Namespace

Ask the user:

> "Do you want to delete the namespace `<NAMESPACE>` itself? This will remove all remaining resources inside it."

Only proceed if the user confirms. Namespace deletion is broad — make sure the user understands it removes everything still in the namespace, not just llm-d resources.

```bash
kubectl delete namespace $NAMESPACE
```

> **Note**: Deleting a namespace is namespace-scoped, not cluster-level. Do NOT delete cluster-wide resources (ClusterRoles, StorageClasses, CRDs, etc.).
...
3. **HTTPRoutes, PVCs, and namespace deletion are always opt-in** — ask separately for each; never assume.
...
3. **Do NOT delete PVCs without explicit user approval** — they may contain valuable benchmark results or cached data.
4. **Do NOT skip the confirmation step** — Even if the user says "just tear it down", show the specific list of resources first.
...
- Partial cleanup (e.g., remove model service only, keep the gateway or PVCs)
